...
import { getallimages } from '../firebase/Firebaseapi';
...
const allmeme =  ({navigation})=>{
 const [transfer,settransfer]= useState([]);
 const [load,setload]=useState(true);
 useEffect(()=>{
  getallimages(settransfer,setload);
  },[]);
...
console.log("123");
   ...
   <View style={{flex:1,width:"100%",flexDirection:"row",}}>
    <Navbar nav={navigation} pop={"white"} tem={"#E25822"} edi={"white"} tren={"white"} star={"white"}/>
   <View style={styles.container}>     
     <View style={{height:40,width:"100%",marginBottom:15,marginTop:15}}>
       <Text style={{color:"white",textAlign:"center",fontWeight:"bold",fontSize:30 }}>ALL  Template</Text>
     </View>
   <FlatList
data={transfer}
numColumns={2}
